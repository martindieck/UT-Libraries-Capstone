...
def main(input_csv_path, output_csv_path):
    ...
    data['Rev_date_normalized'], data['Rev_date_flag'] = zip(*data['Rev. date:'].apply(normalize_date))

    # Select relevant columns to save to a new DataFrame
    #output_data = data[['unique ID', 'Rev. date', 'Rev_date_normalized', 'Rev_date_flag']]
    output_data = data[['unique ID', 'Rev. date:', 'Rev_date_normalized', 'Rev_date_flag']]
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python Rev_date.py <input_csv_path> <output_csv_path>")
        sys.exit(1)
